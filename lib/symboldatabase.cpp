        ...
        for (const Dimension &const_dimension : var->dimensions()) {
            ...
            if (Token::Match(dimension.tok->previous(), "[<,]")) {
                ...
                if (Token::Match(tokenList.front(), "; %num% ;")) {
                    ...
                    dimension.num = MathLib::toLongNumber(tokenList.front()->next());
                }
    ...
    if (container && container->arrayLike_indexOp && container->size_templateArgNo > 0) {
        ...
        if (tok) {
            ...
            if (Token::Match(tok, "%num% [,>]")) {
                ...
                dimension_.num = MathLib::toLongNumber(tok);
            } else if (tok) {
                dimension_.tok = tok;
    ...
    for (Token *tok = tokens; tok; tok = tok->next()) {
        if (tok->isNumber()) {
            if (MathLib::isFloat(tok->str())) {
                ...
                const MathLib::biguint value = MathLib::toULongNumber(tokStr, tok);
